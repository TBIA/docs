...
1. 當需要修訂正式文件時（例如 `title.md`），先確認**之前有無建立過版本**。
    - 若**沒有**建立過版本：先建立一份舊版文件 md 檔（例如 `title-v1.md`）保留現況，再建立 draft 文件。
    - 若**已經**建立過版本：直接建立 draft 文件（例如 `title-draft.md`）。
2. 編輯 draft 文件。
3. 將修改發佈到 Github pages 預覽。
    - 若發現錯誤，回到步驟 2 繼續編輯 draft，再重新預覽，反覆修正直到正確。
4. 確認無誤後**定版**，並同時進行下列兩件事：
    - 將新編輯 draft 的內容覆蓋原本文件（例如覆蓋回 `title.md`）。
    - 更改 draft 的檔名為新版本（例如 `title-draft.md` 改名為 `title-v2.md`）。
5. 兩項都完成後，流程結束。
...
---
flowchart TD
    Start[需要修訂正式文件<br />例如: title.md] --> Check{之前有無建立版本}
    Check --> |是|Draft[建立 draft 文件<br/>例如: title-draft.md]
    Check --> |否|Backup[建立新的舊版文件md檔<br />例如 title-v1.md]
    Backup --> Draft
    Draft --> Edit[編輯draft]
    Edit -->|修改| GHP[Github pages]
    GHP -->|發現錯誤| Edit
    GHP -->Save(定版)
    Save --> Update[將新編輯draft的內容覆蓋原本文件<br />例如 title.md]
    Save --> Rename[更改draft檔名<br />例如 title-v2.md<br />]
    Update --> Done
    Rename --> Done
```
!!! info "說明"
