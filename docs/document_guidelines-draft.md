...
1. 先確定這份文件之後**會不會有版本更新**，並依此在 [/docs](https://github.com/TBIA/docs) 資料夾中新增檔案：
   - **會有版本更新**：新增 **2 個** `.md` 檔案。`my-title.md` 永遠是最新版，`my-title-v1.md` 則保留第一版的內容。
   - **一次性文件**：新增 **1 個** `.md` 檔案 `my-title.md`。
2. 根據本指引與文件模板撰寫該文件檔案。
3. 完成文件撰寫後，通知TBIA技術總監，請秘書處確認是否發佈。
4. 秘書處確認後，由文件工作小組發佈至文件網站（修改mkdocs.yml）。
5. 從TBIA入口網取得該文件的ARK永久網址（persistent URL）。
6. 將ARK網址填回文件md檔的「永久統一資源標籤 Persistent URI」欄位。

```mermaid
flowchart TD
    A(["開始撰寫新文件"]) --> B{"這份文件之後<br/>會不會有版本更新？"}
    B -- "會有版本更新" --> C["新增 2 個 .md 檔案<br/>my-title.md：永遠是最新版<br/>my-title-v1.md：保留第一版內容"]
    B -- "一次性文件" --> D["新增 1 個 .md 檔案<br/>my-title.md"]
    C --> E["依本指引與文件模板<br/>撰寫該文件"]
    D --> E
    E --> F["通知TBIA技術總監<br/>請秘書處確認是否發佈"]
    F --> G["文件工作小組發佈至文件網站<br/>（修改 mkdocs.yml）"]
    G --> H["從TBIA入口網取得該文件的<br/>ARK永久網址（persistent URL）"]
    H --> I["將ARK網址填回文件md檔的<br/>「永久統一資源標籤 Persistent URI」欄位"]
    I --> J(["完成"])
```
